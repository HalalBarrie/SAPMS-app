...
<body class="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-300 min-h-screen" x-data="{ activeTab: 'dashboard', darkMode: true, showHistorical: false, expandedYears: {}, showGPAProjection: false }" x-bind:class="{ 'dark': darkMode }">
    ...
    <div class="container mx-auto px-6 py-8 space-y-8">
        ...
        <!-- Analytics Tab - Magic Bento Style -->
        <div x-show="activeTab === 'analytics'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform scale-100" x-transition:leave-end="opacity-0 transform scale-95">
            <!-- Test Content -->
            <div class="bg-green-500 text-white p-4 rounded-lg mb-4">
                <h3>📊 ANALYTICS TAB IS WORKING!</h3>
                <p>If you can see this green box, the Analytics tab is functioning correctly.</p>
            </div>
            <div class="bento-section">
                <div class="bento-grid">
                    <div class="bento-card text-white" style="grid-column: span 4;">
                        <div class="flex flex-col justify-between h-full">
                            ...
                            <div>
                                <h2 class="text-xl font-bold mb-6 text-white">Performance Analytics</h2>
                                
                                @if($groupedCourses->isEmpty())
                                    <div class="text-center py-12">
                                        <div class="text-6xl mb-4">📊</div>
                                        <h3 class="text-xl font-semibold mb-2 text-white">No data to analyze</h3>
                                        <p class="text-gray-300">Add some courses to see your performance analytics!</p>
                                    </div>
                                @else
                                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        <div>
                                            <h3 class="text-lg font-semibold mb-4 text-gray-300">GPA Trend Over Time</h3>
                                            <div class="bg-white/10 rounded-lg p-4">
                                                <canvas id="gpaChart" width="400" height="200"></canvas>
                                            </div>
                                        </div>
                                        <div>
                                            <h3 class="text-lg font-semibold mb-4 text-gray-300">Grade Distribution</h3>
                                            <div class="bg-white/10 rounded-lg p-4">
                                    ...
                                    <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                                        @php
                                            ...
                                            $averageGPA = \App\Helpers\GPAHelper::calculateGPA($allCourses);
                                            
                                            // Fix best grade logic - find the highest grade (A is highest, F is lowest)
                                            $gradeOrder = ['A' => 5, 'B' => 4, 'C' => 3, 'D' => 2, 'E' => 1, 'F' => 0];
                                            $bestGrade = 'N/A';
                                            $highestValue = -1;
                                            foreach ($allCourses as $course) {
                                                if (isset($gradeOrder[$course->grade]) && $gradeOrder[$course->grade] > $highestValue) {
                                                    $highestValue = $gradeOrder[$course->grade];
                                                    $bestGrade = $course->grade;
                                                }
                                            }
                                            
                                            $improvement = $currentYearGPA['cumulative'] > 3.0 ? 'Improving' : 'Needs Attention';
                                        @endphp
                                        
                                        <div class="bg-blue-600/20 rounded-lg p-4 border border-blue-500/30">
                                            <h4 class="font-semibold text-blue-300 mb-2">Overall Performance</h4>
                                            <p class="text-2xl font-bold text-blue-200">{{ number_format($averageGPA, 2) }}</p>
                                            <p class="text-sm text-blue-300">Average GPA</p>
                                        </div>
                                        
                                        <div class="bg-green-600/20 rounded-lg p-4 border border-green-500/30">
                                            <h4 class="font-semibold text-green-300 mb-2">Best Grade</h4>
                                            <p class="text-2xl font-bold text-green-200">{{ $bestGrade }}</p>
                                            <p class="text-sm text-green-300">Highest Achievement</p>
                                        </div>
                                        
                                        <div class="bg-orange-600/20 rounded-lg p-4 border border-orange-500/30">
                                            <h4 class="font-semibold text-orange-300 mb-2">Trend</h4>
                                            <p class="text-2xl font-bold text-orange-200">{{ $improvement }}</p>
                                            <p class="text-sm text-orange-300">Performance Status</p>
                                        </div>
                                    </div>
                                @endif
                            </div>
        ...
        <!-- Settings Tab - Magic Bento Style -->
